...
# Update path to the java binary
java_path = f'{java_home}/bin/java'

# Set the environment variable and configure nltk
...
year = 2014
remaining_test_path_laptop_og = "data/After_ontology/laptop/original_test.txt"
remaining_test_path_laptop_ARTS = "data/After_ontology/laptop/ARTS_test.txt"
remaining_test_path_laptop_ARTS_PLUS_PLUS = "data/After_ontology/laptop/ARTS++_test.txt"
...
predictions_ontology_file_laptop_og = "data/ontology_preds/laptop/original_test.txt"
predictions_ontology_file_laptop_ARTS = "data/ontology_preds/laptop/ARTS_test.txt"
predictions_ontology_file_laptop_ARTS_PLUS_PLUS = "data/ontology_preds/laptop/ARTS_PLUS_PLUS_test.txt"
...
class OntReasoner():
    ...
    def run(self, use_backup, path, use_svm, cross_val=False, j=0):
        ...
        if use_backup:
            ...
            outF = open(remaining_test_path_laptop_ARTS_PLUS_PLUS, "w")
            pred_ontF = open(predictions_ontology_file_laptop_ARTS_PLUS_PLUS, "w")
            with open(test_path_laptop_ARTS_PLUS_PLUS, "r") as fd:
                for i, line in enumerate(fd):
                    if i in self.remaining_pos_vector:
                        outF.write(line)
            outF.close()
            current_index = 0
            for index in self.posinfo:
                if index in self.remaining_pos_vector:
                    pred_ontF.write("Unsure\n")
                else:
                    pred_ontF.write(str(bool_vector[current_index]) + "\n")
                    current_index += 1
            pred_ontF.close()
